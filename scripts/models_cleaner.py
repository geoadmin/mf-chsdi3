...
def rename_template_files(model_file_path: str, dry_run: bool = True):
    """
    Rename __template__ filenames based on the corresponding __bodId__ value.
    Example:
      __bodId__ = 'ch.are.landschaftstypen'
      __template__ = 'templates/htmlpopup/landschaftstypen.mako'
    becomes:
      __template__ = 'templates/htmlpopup/ch_are_landschaftstypen.mako'

    Args:
        model_file_path: Path to the .py model file
        dry_run: If True, only print changes (don’t modify file)
    """
    ...
    bodid_re = re.compile(r"__bodId__\s*=\s*['\"]([^'\"]+)['\"]")
    template_re = re.compile(r"(__template__\s*=\s*['\"])(templates/htmlpopup/)([^'\"]+)(['\"])")

    for line in lines:
        bodid_match = bodid_re.search(line)
        template_match = template_re.search(line)

        if bodid_match:
            current_bodid = bodid_match.group(1)
        elif template_match and current_bodid:
            bodid_name = current_bodid.replace(".", "_")
            new_template_name = f"{template_match.group(2)}{bodid_name}.mako"
            new_line = f"{template_match.group(1)}{new_template_name}{template_match.group(4)}\n"

            if new_line != line:
                print(f"Updating template → {new_template_name}")
                line = new_line
