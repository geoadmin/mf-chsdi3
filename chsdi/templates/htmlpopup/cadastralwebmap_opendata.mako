...
<%
  ...
  shp_url = "%s/ch.swisstopo-vd.amtliche-vermessung/DM01AVCH24D/SHP/%s/%s.zip" % (request.registry.settings['datageoadminhost'], c['attributes']['ak'],c['attributes']['bfsnr'])
  itf_url = "%s/ch.swisstopo-vd.amtliche-vermessung/DM01AVCH24D/ITF/%s/%s.zip" % (request.registry.settings['datageoadminhost'], c['attributes']['ak'],c['attributes']['bfsnr'])
%>
...
<%
    ...
    request = context.get('request')
    download_url = request.params.get('download_url')
    pdf = False

    if download_url:
        try:
            # Only accept URLs from the PDF order service
...
<script>
    $(document).ready(function() {
        ...
        $('#pdf_download').html('<p>${_("ch.swisstopo-vd.amtliche-vermessung.waitpdf")}<br/><br/><img src="${request.static_url("chsdi:static/images/loading.gif")}" width="40px" />');
        (function next() {
