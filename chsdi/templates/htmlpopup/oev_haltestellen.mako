...
% if type_station == 'Bedienpunkt' :
    ...
    <p>${h.translate('ch.bav.haltestellen-oev.bedienpunkt', lang)}</p>

% elif type_station == 'Anschlusspunkt' :

   <p><b>${c['attributes']['name'] or '-'}</b></p>
   <p>${h.translate('ch.bav.haltestellen-oev.anschlusspunkt', lang)}</p>

% elif type_station == 'reiner_Betriebspunkt' :

   <p><b>${c['attributes']['name'] or '-'}</b></p>
   <p>${h.translate('ch.bav.haltestellen-oev.reiner_betriebspunkt', lang)}</p>
...
$(document).ready(function() {
  ...
  var getInfos = function(val){
    ...
    $.getJSON( '${host}/stationboard/stops/${id}', function(result){
      ...
      destinationCol.html('<p style="height:24px; margin:0px;">${h.translate('ch.bav.haltestellen-oev.nodata2', lang)}</p>');
    });
  };
  ...
  Refresh();
    ...
    <p><b>${c['attributes']['name'] or '-'}</b>, ${h.translate('ch.bav.haltestellen-oev.next_departures', lang)}:</p>
  <table>
    <tr>
...
<table>
  <tr>
      <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.id', lang)}</td>
      <td class="cell-meta">${c['featureId'] or '-'}</td>
  </tr>
  <tr>
    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.name', lang)}</td>
    <td class="cell-meta">${c['attributes']['name'] or '-'}</td>
  </tr>
  <tr>
    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.abkuerzung', lang)}</td>
    <td class="cell-meta">${c['attributes']['abkuerzung'] or '-'}</td>
  </tr>
  <tr>
    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.tuabkuerzung', lang)}</td>
    <td class="cell-meta">${c['attributes']['tuabkuerzung'] or '-'}</td>
  </tr>
  <tr>
    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.betriebspunkttyp', lang)}</td>

% if type_station == 'Bedienpunkt' :

    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.bedienpunkt', lang)}</td>

% elif type_station == 'Anschlusspunkt' :

   <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.anschlusspunkt', lang)}</td>

% elif type_station == 'reiner_Betriebspunkt' :

   <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.reiner_betriebspunkt', lang)}</td>

% elif type_station == 'Haltestelle' :

    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.haltestelle', lang)}</td>

% endif
  </tr>
  <tr>
    <td class="cell-meta">${h.translate('ch.bav.haltestellen-oev.verkehrsmittel', lang)}</td>
    <td class="cell-meta">${h.translate(verkehr, lang)}</td>
  </tr>
  <tr>
