...
def get_xml(path):
    ...
    try:
      ...
      if response.status_code == 200:
       ...
       xmlstring = re.sub(r'(\sxmlns|\sxsi)[^"]+"[^"]+"', '', response.text)
...
%>
    ...
    <tr><td class="cell-left">${t.Translator.translate('gemgemeinde', lang)}</td>    <td>${c['attributes']['gemeindename'] or '-'}</td></tr>
    <tr><td class="cell-left">${t.Translator.translate('oereb_status', lang)}</td>
% if lang == 'de':
...
% endif
    <tr><td class="cell-left">${t.Translator.translate('oereb_firma', lang)}</td>    <td>${c['attributes']['firmenname'] or '-'}</td></tr>
    <tr><td class="cell-left">${t.Translator.translate('grundadresse', lang)}</td>
    ...
    <tr><td class="cell-left">${t.Translator.translate('grundtel', lang)}</td>    <td>${c['attributes']['telefon'] or '-'}</td></tr>
    <tr><td class="cell-left">${t.Translator.translate('Email', lang)}</td>
    ...
    <tr><td class="cell-left">${t.Translator.translate('gemdarstellung', lang)}</td>
      % if c['attributes']['url_oereb'] == None:
       <td>-</td>
      % else:
         <td><a target="_blank" href="${c['attributes']['url_oereb']}">${t.Translator.translate('link', lang)}</a></td>
      % endif
    </tr>
    % if is_oereb_service:
      % for egrid in list_egrid:
        <tr>
            <td class="cell-left">${t.Translator.translate('ch.swisstopo-vd.stand-oerebkataster.oereb_webservice', lang)}</td>
