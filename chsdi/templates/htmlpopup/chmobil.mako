...
%>
    <tr><td class="cell-left">${Translator.translate('chmobil_number', lang)}</td>   <td>${c['featureId'] or '-'}</td></tr>
    <tr><td class="cell-left">${_(msgid_route)}</td>    <td>${c['attributes']['chmobil_title'] or '-'}</td></tr>
    <tr><td class="cell-left">${Translator.translate('link', lang)}</td>          <td>
    % if url and c['attributes']['chmobil_route_number']:
      <a href="${url}/route${c['attributes']['chmobil_route_number']}" target="_blank" title="${Translator.translate('chmobil_url_route')}">${_('chmobil_url_route', lang)}</a>
    % else:
        -
    % endif
    </td></tr>
    ...
    % if url and c['attributes']['chmobil_has_segment']:
      <a href="${url}/etappe${c['featureId']}" target="_blank" title="${Translator.translate('chmobil_url_route')}">${_('chmobil_url_etappe', lang)}</a>
    % else:
        -
    % endif
    </td></tr>
