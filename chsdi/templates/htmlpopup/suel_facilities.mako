...
%>
    ...
    <tr><td class="cell-left">${h.translate('tt_sachplan_beschreibung', lang)}</td>             <td>${c['attributes'][description_text] or '-' | n}</td></tr>
% if c['attributes']['doc_web']:
    <tr><td class="cell-left">${h.translate(doc_title, lang)}</td>                              <td><a href="${c['attributes']['doc_web']}" target="_blank">${c['attributes']['doc_title'] or '-'}</a></td></tr>
% else:
