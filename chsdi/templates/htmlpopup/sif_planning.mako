<%inherit file="base.mako"/>

<%def name="table_body(c, lang)">
<%
    name = c['layerBodId'] + '.' + 'plname_%s' % lang
    doc_title = c['layerBodId'] + '.' + 'doc_title'
%>

<%
import datetime
lang = lang if lang in ('fr','it') else 'de'
plname = 'plname_%s' % lang
facname = 'facname_%s' % lang
measuretype_text = 'meastype_text_%s' % lang
coordinationlevel_text = 'coordlevel_text_%s' % lang
planningstatus_text = 'plstatus_text_%s' % lang
description_text = 'description_%s' % lang
dateto = '-'
datefrom = datetime.datetime.strptime(c['attributes']['validfrom'].strip(), "%Y-%m-%d").strftime("%d.%m.%Y")
if c['attributes']['validuntil']:
    dateto = datetime.datetime.strptime(c['attributes']['validuntil'].strip(), "%Y-%m-%d").strftime("%d.%m.%Y")
endif
%>
    <tr>
      <td class="cell-left">${t.Translator.translate('ch.bav.sachplan-infrastruktur-schiene_anhorung.plname_de', lang)}</td>
      <td>${c['attributes'][plname]}</td>
    </tr>
	  <tr>
      <td class="cell-left">${t.Translator.translate('tt_sachplan_facility_beschlussdatum', lang)}</td>
      <td>${datefrom or '-'}</td>
    </tr>
% if c['attributes']['doc_web']:
    <tr>
      <td class="cell-left">${t.Translator.translate('ch.bav.sachplan-infrastruktur-schiene_anhorung.doc_title', lang)}</td>
      <td><a href="${c['attributes']['doc_web']}" target="_blank">${c['attributes']['doc_title'] or '-'}</a></td></tr>
% else:
    <tr>
      <td class="cell-left">${t.Translator.translate('tt_sachplan_weitereinfo', lang)}</td>
      <td> - </td>
    </tr>
%endif
</%def>
