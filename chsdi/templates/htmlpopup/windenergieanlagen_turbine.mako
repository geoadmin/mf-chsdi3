...
<%def name="table_body(c, lang)">
 <tr>
  <td class="cell-left">${t.Translator.translate('windenergieanlage', lang)}</td>
  <td>${c['attributes']['fac_name'] or '-'}</td>
 </tr>
 <tr>
  <td class="cell-left">${t.Translator.translate('elevation', lang)}</td>
  <td>${c['attributes']['tur_altitude'] or '-'} ${t.Translator.translate('abk_meter_ueber_meer', lang)}</td>
 </tr>
 <tr>
  <td class="cell-left">${t.Translator.translate('leistung', lang)}</td>
 ...
 <tr>
  <td class="cell-left">${t.Translator.translate('baujahr', lang)}</td>
  <td>${c['attributes']['tur_year'] or '-'}</td>
 </tr>
</%def>

<%def name="extended_info(c, lang)">
<title>${t.Translator.translate('ch.bfe.windenergieanlagen', lang)}</title>
<body onload="init()">
<div class="zsborder">
<%
...
%>   
<h3>${t.Translator.translate('windenergieangaben', lang)}</h3>
<table class="table-with-border kernkraftwerke-extended" cellpadding="5" width=100%>
  <tr>
    <th class="cell-meta" width=30%>
      ${t.Translator.translate('tt_ch.bfe.abgeltung-wasserkraftnutzung_name', lang)}
    </th>
    <td width=70%>
  ...
  <tr>
    <th class="cell-meta">${t.Translator.translate('windenergiebetreiber', lang)}</th>
    <td>
      ${c['attributes']['fac_operator']}
    </td>
  </tr>
  <tr>
    <th class="cell-meta">${t.Translator.translate('website', lang)}</th>
  ...
  <tr>
    <th class="cell-meta">${t.Translator.translate('installierteleistung', lang)}</th>
...
<h3> ${t.Translator.translate('produktionsdaten', lang)}</h3>
% if c['attributes']['fac_xml_prod'] != None:
<table class="table-with-border kernkraftwerke-extended" cellpadding="5" width=100%>
  <tr width=100%>
    <th class="cell-meta" width=20%>
      ${t.Translator.translate('tt_emission_datum', lang)}
    </th>
    <th class="cell-meta" width=80%>
      ${t.Translator.translate('produktion', lang)}
    </th>
  </tr>
...
src="http://www.uvek-gis.admin.ch/BFE/bilder/ch.bfe.windenergieanlagen/chart_${c['attributes']['fac_initial']}.png" alt=""/></tr>
</table>
% else:
<h3> - </h3>
% endif
<h3> ${t.Translator.translate('windturbinenangaben', lang)}</h3>
...
<tr>
  <th class="cell-meta">${t.Translator.translate('hersteller', lang)}</th>
  <th class="cell-meta">${t.Translator.translate('modell', lang)}</th>
  <th class="cell-meta">${t.Translator.translate('rotordurchmesser', lang)}</th>
  <th class="cell-meta">${t.Translator.translate('nabenhoehe', lang)}</th>
  <th class="cell-meta">${t.Translator.translate('installierteleistung', lang)}</th>
  <th class="cell-meta">${t.Translator.translate('baujahr_de_fr', lang)}</th>
  <th class="cell-meta">${t.Translator.translate('statut', lang)}</th>
</tr>
% if lang=='fr':
% for tu in turbs:
...
<%def name="extended_resources(c, lang)">
  <script type="text/javascript" src="${h.get_loaderjs_url(request, '4.4.2')}"></script>
</%def>
