...
<%
    arr_fn_zentrale_144 = c['attributes']['fn_zentrale_144'].split('\n')
    ...
    str_output_mo = ''
    for i in range(arr_len):
        str_output_mo = str_output_mo + arr_mo_zentrale_144[i] + '<br />' if  i < (arr_len-1) else str_output_mo + arr_mo_zentrale_144[i]
    endfor
%>

<% c['stable_id'] = True %>

    <tr><td class="cell-left">${_('gemeinde')}</td>    	<td>${c['attributes']['gemeinde_144'] or '-'}</td></tr>
