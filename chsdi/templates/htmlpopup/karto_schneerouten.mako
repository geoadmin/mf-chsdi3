...
%>
    % if c['attributes']['target_name']:
        <% c['stable_id'] = True %>
        <tr><td class="cell-left">${_('ch.swisstopo-karto.skitouren.summit')}</td>           <td>${c['attributes']['target_name'] or '-'} (${c['attributes']['target_altitude'] or '-'} m)</td></tr>
        ...
        % if c['attributes'][url_sac]:
            <tr><td class="cell-left">${_('ch.swisstopo-karto.skitouren.info')}</td>         <td><a target="_blank" href="${c['attributes'][url_sac]}">${_('ch.swisstopo-karto.skitouren.sac')}</a></td></tr>
        % else:
