<%def name="init_map(ebkey, width, height, rotation, target)">
        ...
        */
        ...
        function getEqualizedValue(histogram, l){
          var equalizedValue = 0;

          for (var i = 0; i < (l + 1); i++){
            equalizedValue += histogram[i];
          }

          return equalizedValue;
        }

        function getPixelValue(degree, pixel, mean, histogram){
          var hcl = rgb2hcl(pixel);
          var l = hcl[2];
          var equalizedValue = getEqualizedValue(histogram, l);
          var new_l;
          ...
          if (l == 0 || degree == 100){
            ...
            new_l = l * (1 - (degree / 100)) + ((degree / 100)) * 100 * equalizedValue;
          }
        ...
        var raster = new ol.source.Raster({
          ...
          operation: function(pixels, data) {
            ...
            if (data.histogram && pixel[3]!=0){
              var new_pixel = getPixelValue(degree_contrast, pixel, data.mean, data.histogram);
              pixel[0] = new_pixel[0];
              pixel[1] = new_pixel[1];
              pixel[2] = new_pixel[2];
            }
          ...
          lib:{
            ...
            getEqualizedValue: getEqualizedValue,
            rgb2hcl: rgb2hcl,
            hcl2rgb: hcl2rgb,
            rgb2xyz: rgb2xyz,
            lab2xyz: lab2xyz,
