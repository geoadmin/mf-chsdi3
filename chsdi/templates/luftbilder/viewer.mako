...
<html>
  ...
  <body onload="init()">
    ...
    <div class="wrapper">
      <div id="lubismap">
        <button id="contrast_activate" class="button btn btn-secondary btn-sm d-block">${h.translate('image_contrast_activate', lang)}</button>
      </div>
    </div>
    <div class="footer">
  ...
  % if displayLink:
      <div class="pull-right">
        <a class="link-red" href="${''.join((baseUrl, '?', layerBodId, '=', str(featureId), '&lang=', lang, '&topic=', topic))}" target="new">${h.translate('Link to object', lang)}</a>
      </div>
  % endif
    </div>
       ...
       function getPixelList(data){
        ...
        function onButtonChange() {
          if (contrastLayer.getVisible() == true){
            ...
            contrastButtonActivate.innerHTML = "${h.translate('image_contrast_activate', lang)}";
            setContrastPermalink('false')
          } else{
            contrastLayer.setVisible(true);
            contrastButtonActivate.innerHTML = "${h.translate('image_contrast_deactivate', lang)}";
