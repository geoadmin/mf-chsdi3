...
| **lang (optional)**               | The language. Supported values: de, fr, it , rm, en. Defaults to "de".                    |
+-----------------------------------+-------------------------------------------------------------------------------------------+
| **where (optional)**              | Filter features with an expression only. Do not need a spatial filter (`mapExtent`,       |
|                                   | `tolerance`, `sr`, `geometry`, `geometryType` and `imageDisplay`)                         |
|                                   | Mutually exclusive with parameter `layerDefs`                                             |
+-----------------------------------+-------------------------------------------------------------------------------------------+
| **layerDefs (optional)**          | Filter features with an expression.                                                       |
|                                   | Syntax: `{ "<layerId>" : "<layerDef1>" , "<layerId>" : "<layerDef2>"  }`                  |
|                                   | Mutually exclusive with parameter `where`                                                 |
+-----------------------------------+-------------------------------------------------------------------------------------------+
| **callback (optional)**           | The name of the callback function.                                                        |
+-----------------------------------+-------------------------------------------------------------------------------------------+
...
You may filter by attributes with **where** or **layerDefs** on `queryable layers <../api/faq/index.html#which-layers-are-queryable>`_.
...
+=================+======================================+================================================================+
|  varchar        |  =, like, ilike, not like, not ilike |  toto ='3455 Kloten', toto ilike '%SH%', toto is null          |
|                 |  is null, is not null                |  toto ilike 'SH%'                                              |
+-----------------+--------------------------------------+----------------------------------------------------------------+
...
- Filter features with **where**: `https://api3.geo.admin.ch/rest/services/all/MapServer/identify?geometryFormat=geojson&imageDisplay=1367,949,96&lang=en&layers=all:ch.bazl.luftfahrthindernis&mapExtent=2318250,952750,3001750,1427250&returnGeometry=true&sr=2056&tolerance=5&where=bgdi_activesince >= '2019-04-30' and state ilike '%A%' <../../../rest/services/all/MapServer/identify?geometryFormat=geojson&imageDisplay=1367,949,96&lang=en&layers=all:ch.bazl.luftfahrthindernis&mapExtent=2318250,952750,3001750,1427250&returnGeometry=true&sr=2056&tolerance=5&where=bgdi_activesince+%3E%3D+%272019-04-30%27+and+state+ilike+%27%25A%25%27>`_
