//(function() {
  ...
  goog.provide('ga.StylesFromLiterals');
  ...
  //module.provider('gaStylesFromLiterals', function() {
    ...
    ga.StylesFromLiterals = function(properties) {
      ...
      function getOlStyleForPoint(options, shape) {
        if (shape === 'circle') {
          ...
          var style = $.extend({}, shapes[shape], options);
      ...
      function getOlStyleFromLiterals(value) {
        ...
        $.each(style, function(key, value) {
          ...
          if (key === 'image') {
            ...
            options = $.extend({}, styleP, options);
            olStyle = getOlStyleForPoint(options, value.type);
            olStyles[key] = olStyle;
          } else {
            olStyles = $.extend({}, olStyle, getOlBasicStyles(style));
          }
        });
